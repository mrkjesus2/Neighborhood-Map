...
app.Map = {
  ...
  init: function() {
    ...
    this.status = google.maps.places.PlacesServiceStatus;
  ...
  getPlaces: function() {
    ...
    this.placesApi.nearbySearch(request, function(results, status) {
      if (status === 'OK') {
        results.forEach(function(result, idx) {
          // app.Map.places.push(results[idx]);
  ...
  getPlaceDetails: function(place) {
    ...
    this.placesApi.getDetails(request, function(loc, status) {
      if (status === 'OK') {
        return loc;
      } else {
        console.log(status);  // TODO: Add UI error handling
  ...
  createMarker: function(place) {
    ...
    google.maps.event.addListener(marker, 'click', function() {
      // TODO: Add infoWindow, preferably with 3rd Party API calls
    });
